...
class Fw_Updata():
    ...
    def init_nic_dev(self):
        ...
        params :none
        '''
        if not self.check_command(cmdlist=['mst','flint','ethtool']):
            self.clear_cmd()
            sys.exit("FW updata command not find please check log %s" % self.runlogfile)
        else:
            os.system('mst start')
            mstinfo = subprocess.getstatusoutput('mst status')[1].split("\n")
            mstlist = []
            for infotmp in mstinfo:
                if infotmp == "":
                    continue
                else:
                    mstlist.append(infotmp.strip())
            for k,v in self.nic_port_info.items():
                for minfo in copy.deepcopy(mstlist):
                    if re.search(r'%s' % v, minfo, re.I):
                        listid = mstlist.index(minfo) - 1
            if re.search(r'/dev/mst/\S*',mstlist[listid],re.I):
                self.nicdev = re.search(r'/dev/mst/\S*',mstlist[listid],re.I).group(0)
                self.nicfwlogger.info(f"Find mst device {self.nicdev}")
                print(f"Find mst device {self.nicdev}")
            else:
                self.nicfwlogger.error(f"Not Find Mst Device Use Command mst status")
                self.clear_cmd()
                sys.exit(f"Not Find Mst Device Use Command mst status")
    ...
    def first_setup(self):
        ...
        cyclecmd = 'cd %s && python3 nic_fw_updata.py -t %s -w %s -i %s -p "%s" -u %s -d %s' % (scriptdir,self.nictype,self.withio,self.checkip,self.nicport,self.upfwfile,self.downfwfile)
    ...
    def Fw_Updata_Fun(self,action):
        ...
        if self.checkip != "None" and self.withio == "True":
            ...
            pingt.start()
        if action == 'up':
            self.nicfwlogger.info("flint --yes -d %s -i %s b\n" % (self.nicdev,self.upfwfile))
            print("flint --yes -d %s -i %s b\n" % (self.nicdev,self.upfwfile))
            update_res = subprocess.getstatusoutput("flint --yes -d %s -i %s b" % (self.nicdev,self.upfwfile))[1]
            self.nicfwlogger.info(update_res)
        if action == 'down':
            self.nicfwlogger.info("flint --yes -d %s -i %s b\n" % (self.nicdev,self.downfwfile))
            print("flint --yes -d %s -i %s b\n" % (self.nicdev,self.downfwfile))
            update_res = subprocess.getstatusoutput("flint --yes -d %s -i %s b" % (self.nicdev,self.downfwfile))[1]
            self.nicfwlogger.info(update_res)
        os.system('reboot')
