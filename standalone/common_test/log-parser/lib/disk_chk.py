...
class Disk_Check():
    ...
    def smartlog_save(self):
        ...
        if self.hddlist:
            ...
            with open("%s/diskinfo.log" % self.diskdir , 'w') as f:
                ...
                f.write('\n')
        if self.nvmelist:
            nvme_cmd = subprocess.getstatusoutput('command -v nvme')[1]
            if nvme_cmd == "":
                print("No nvme tool please install")
                sys.exit()
            else:
                for nvme in self.nvmelist:
                    subprocess.getstatusoutput('nvme smart-log /dev/%s >%s/%s.log' % (nvme,self.diskdir,nvme))
        else:
            with open("%s/diskinfo.log" % self.diskdir , 'w') as f:
    ...
    def is_ssd_or_sata(self,dev):
        ...
        if checktmp == 'Solid State Device':
            m2tmp = subprocess.getstatusoutput("smartctl -a /dev/%s |grep -i 'Form Factor:'" % dev)[1].split(":")[-1].strip()
            if m2tmp == "M.2":
                return 0
            else:
                return 1
            ...
            for hddtmp in self.hddlist:
                ...
                for bkey in hddblack_l:
                    if os.path.exists('%s/diskinfo/after/%s.log' % (self.logdir,hddtmp)) and os.path.exists('%s/diskinfo/before/%s.log' % (self.logdir,hddtmp)):
                        hafinfo = subprocess.getstatusoutput('cat %s/diskinfo/after/%s.log |grep -i "%s"' % (self.logdir,hddtmp,bkey))[1]
                        hbeinfo = subprocess.getstatusoutput('cat %s/diskinfo/before/%s.log |grep -i "%s"' % (self.logdir,hddtmp,bkey))[1]
                        if hafinfo != hbeinfo:
                            errmsg.append("Check smart info fail in %s \nbefore: %s ; \nafter %s" % (hddtmp,hbeinfo,hafinfo))
                            checkflag = False
                            info = "Check smart info fail in %s \nbefore: %s ; \nafter %s" % (hddtmp,hbeinfo,hafinfo)
                            self.save_checklog(state='fail',msg=info)
                        else:
                            if hafinfo != '' and hbeinfo != '':
                                info = "Check %s Smart Info Success \nBefore : %s \nAfter : %s" % (hddtmp,hbeinfo,hafinfo)
            ...
            for nvmetmp in self.nvmelist:
                for nkey in nvmeblack_l:
                    if os.path.exists('%s/diskinfo/after/%s.log' % (self.logdir,nvmetmp)) and os.path.exists('%s/diskinfo/before/%s.log' % (self.logdir,nvmetmp)):
                        ...
                        if nafinfo != nbeinfo:
                            errmsg.append("Check smart info fail in %s \nbefore: %s ; \nafter %s" % (nvmetmp,nbeinfo,nafinfo))
                            checkflag = False
                            info = "Check smart info fail in %s \nbefore: %s ; \nafter %s" % (nvmetmp,nbeinfo,nafinfo)
                            self.save_checklog(state='fail',msg=info)
                        else:    
